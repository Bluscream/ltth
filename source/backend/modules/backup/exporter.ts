 ...
 */
export async function exportBackup(deps: any, opts: any = {}) {
    const {
        db,
        configPathManager,
        pluginLoader,
    ...
    if (includePluginSettings || includePluginData) {
        ...
        for (const pluginId of selectedIds) {
            ...
            const provider = backupProviders[pluginId as string];
            ...
            if (customExport && customExport.warnings) {
                warnings.push(...customExport.warnings.map((w: any) => `[${pluginId}] ${w}`));
            }

            // Merge: custom settings override generic DB settings
            const dbSettings = rawPluginSettings[pluginId as string] || {};
            const finalSettings = customExport && customExport.settings
                ? { ...dbSettings, ...customExport.settings }
                : dbSettings;

            allPluginSettings[pluginId as string] = finalSettings;
