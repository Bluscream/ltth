...
let nodeModulesPath = path.join(appDir, 'node_modules');

// Fallback to project root node_modules if not found in current dir
if (!fs.existsSync(nodeModulesPath) && fs.existsSync(path.join(appDir, '../../node_modules'))) {
    nodeModulesPath = path.join(appDir, '../../node_modules');
}
...
(Module as any)._nodeModulePaths = function(from: string) {
  const paths = originalNodeModulePaths.call(this, from) as string[];
