const myPluginSocket = window.socket || io();let currentConfig = {};

// Status-Updates empfangen
myPluginSocket.on('osc:status', (status) => {
    updateStatus(status);
});

// OSC-Nachrichten loggen (wenn verbose)
myPluginSocket.on('osc:sent', (data) => {
    if (currentConfig.verboseMode) {
        addLog('send', `${data.address} → ${JSON.stringify(data.args)}`);
    }
});

myPluginSocket.on('osc:received', (data) => {
    if (currentConfig.verboseMode) {
        addLog('recv', `${data.address} ← ${JSON.stringify(data.args)} from ${data.source}`);
    }
});

// Auto-update gift catalog when new gifts are detected from stream
myPluginSocket.on('gift-catalog:updated', (data) => {
    if (data && data.gift && data.gift.id != null) {
        // Check if gift is already in local catalog (handle both string and number IDs)
        const newGiftId = typeof data.gift.id === 'number' ? data.gift.id : parseInt(data.gift.id, 10);
        if (isNaN(newGiftId)) return; // Invalid ID, skip
        
        const existingIndex = giftCatalog.findIndex(g => {
            if (g.id == null) return false;
            const existingId = typeof g.id === 'number' ? g.id : parseInt(g.id, 10);
            return !isNaN(existingId) && existingId === newGiftId;
        });
        
        if (existingIndex === -1) {
            // Add new gift to local catalog
...
function populateForm(config) {
    ...
    if (config.oscQuery) {
        const oscQueryEnabled = document.getElementById('oscQueryEnabled');
        const oscQueryHost = document.getElementById('oscQueryHost');
        const oscQueryPort = document.getElementById('oscQueryPort');
        const oscQueryAutoSubscribe = document.getElementById('oscQueryAutoSubscribe');
        
        if (oscQueryEnabled) oscQueryEnabled.checked = config.oscQuery.enabled || false;
        if (oscQueryHost) oscQueryHost.value = config.oscQuery.host || '127.0.0.1';
        if (oscQueryPort) oscQueryPort.value = config.oscQuery.port || 9001;
        if (oscQueryAutoSubscribe) oscQueryAutoSubscribe.checked = config.oscQuery.autoSubscribe ?? true;
    }
    
    if (config.liveMonitoring) {
        const liveMonitoringEnabled = document.getElementById('liveMonitoringEnabled');
        const liveMonitoringInterval = document.getElementById('liveMonitoringInterval');
        const liveMonitoringHistory = document.getElementById('liveMonitoringHistory');
        
        if (liveMonitoringEnabled) liveMonitoringEnabled.checked = config.liveMonitoring.enabled || false;
        if (liveMonitoringInterval) liveMonitoringInterval.value = config.liveMonitoring.updateInterval || 100;
        if (liveMonitoringHistory) liveMonitoringHistory.value = config.liveMonitoring.historyDuration || 60000;
    }
    
    if (config.physBones) {
        const physBonesEnabled = document.getElementById('physBonesEnabled');
        if (physBonesEnabled) physBonesEnabled.checked = config.physBones.enabled || false;
    }
    
    if (config.messageBatching) {
        const messageBatchingEnabled = document.getElementById('messageBatchingEnabled');
        const batchWindow = document.getElementById('batchWindow');
        
        if (messageBatchingEnabled) messageBatchingEnabled.checked = config.messageBatching.enabled ?? true;
        if (batchWindow) batchWindow.value = config.messageBatching.batchWindow || 10;
    }
    
    if (config.parameterCaching) {
        const parameterCachingEnabled = document.getElementById('parameterCachingEnabled');
        const cacheTTL = document.getElementById('cacheTTL');
        
        if (parameterCachingEnabled) parameterCachingEnabled.checked = config.parameterCaching.enabled ?? true;
        if (cacheTTL) cacheTTL.value = config.parameterCaching.ttl || 5000;
    }
...
// Advanced Features form handler
const advancedFeaturesForm = document.getElementById('advanced-features-form');
if (advancedFeaturesForm) {
    advancedFeaturesForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        // Build updated config with advanced features
        const updatedConfig = {
            ...getCurrentConfigData.config,
            oscQuery: {
                enabled: oscQueryEnabled ? oscQueryEnabled.checked : false,
                host: oscQueryHost ? oscQueryHost.value : '127.0.0.1',
                port: oscQueryPort ? parseInt(oscQueryPort.value) : 9001,
                autoSubscribe: oscQueryAutoSubscribe ? oscQueryAutoSubscribe.checked : true
            },
            liveMonitoring: {
                enabled: liveMonitoringEnabled ? liveMonitoringEnabled.checked : false,
                updateInterval: liveMonitoringInterval ? parseInt(liveMonitoringInterval.value) : 100,
                historyDuration: liveMonitoringHistory ? parseInt(liveMonitoringHistory.value) : 60000
            },
            physBones: {
                ...(getCurrentConfigData.config.physBones || {}),
                enabled: physBonesEnabled ? physBonesEnabled.checked : false
            },
            messageBatching: {
                enabled: messageBatchingEnabled ? messageBatchingEnabled.checked : true,
                batchWindow: batchWindow ? parseInt(batchWindow.value) : 10
            },
            parameterCaching: {
                enabled: parameterCachingEnabled ? parameterCachingEnabled.checked : true,
                ttl: cacheTTL ? parseInt(cacheTTL.value) : 5000
            }
        };
...
myPluginSocket.emit('osc:get-status');
setInterval(() => {
    myPluginSocket.emit('osc:get-status');
}, 2000);
...
function populateGiftCatalogSelector() {
    ...
    const countEl = document.getElementById('gift-catalog-count');
    const hintEl = document.getElementById('gift-catalog-hint');
...
if (typeof window !== 'undefined') {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // Add event listeners for avatar auto-discovery
        const btnAutoDetectAvatar = document.getElementById('btn-auto-detect-avatar');
        if (btnAutoDetectAvatar) {
            btnAutoDetectAvatar.addEventListener('click', autoDetectAvatar);
        }
...
async function autoDetectAvatar() {
    ...
    try {
        ...
        if (data.success) {
            ...
            displayCurrentAvatar(currentAvatarData);
...
async function refreshAvailableActions() {
    const btn = document.getElementById('btn-refresh-actions');
    const originalText = btn.textContent;
    ...
    try {
        ...
        const actionsResponse = await fetch('/api/osc/avatar/available-actions');
...
function displayCurrentAvatar(avatarData) {
    const container = document.getElementById('current-avatar-info');
    const avatarIdEl = document.getElementById('current-avatar-id');
    const paramCountEl = document.getElementById('current-avatar-param-count');
    
    if (container && avatarIdEl && paramCountEl) {
        container.style.display = 'block';
        avatarIdEl.textContent = avatarData.avatarId || '-';
        paramCountEl.textContent = avatarData.parameterCount || 0;
    }
}
...
function displayAvailableActions(actions) {
    ...
    const container = document.getElementById('available-actions-container');
    if (container) {
        container.style.display = 'block';
    }
    
    // Display standard actions
    displayStandardActions(actions.standard || {});
    
    // Display emote slots
    displayEmoteSlots(actions.emotes || {});
    
    // Display custom parameters
    displayCustomParameters(actions.custom || []);
    
    // Display PhysBones
    displayPhysBones(actions.physbones || []);
}

function displayStandardActions(standardActions) {
    const container = document.getElementById('standard-actions-list');
    if (!container) return;
...
function displayEmoteSlots(emoteSlots) {
    const container = document.getElementById('emote-slots-list');
    if (!container) return;
...
function displayCustomParameters(customParams) {
    const container = document.getElementById('custom-params-list');
    const section = document.getElementById('custom-params-section');
...
function displayPhysBones(physbones) {
    const container = document.getElementById('physbones-list');
    const section = document.getElementById('physbones-section');
...
async function sendCustomParameter(path, value) {
    await sendCustomOSC(path, value, 2000);
}

async function triggerPhysBone(boneName) {
    try {
        const response = await fetch('/api/osc/physbones/trigger', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                boneName: boneName,
        ...
        if (!data.success) {
            console.error('Error triggering PhysBone:', data.error);
        }
    } catch (error) {
        console.error('Error triggering PhysBone:', error);
    }
}
