...
const sbSocket = window.socket || io();

// Add connection status logging
sbSocket.on('connect', () => {
    log.info('Socket.io connected', { id: sbSocket.id });
    
    // Identify as dashboard client for preview sound support
    sbSocket.emit('soundboard:identify', { client: 'dashboard' });
    log.info('Sent identification as dashboard client');
});

sbSocket.on('soundboard:identified', (data) => {
    log.info('Identified by server', data);
});

sbSocket.on('disconnect', (reason) => {
    log.warn('Socket.io disconnected', { reason });
});

sbSocket.on('connect_error', (error) => {
...
// ========== SOCKET EVENTS ==========
sbSocket.on('soundboard:play', (data) => {
...
sbSocket.on('soundboard:preview', (payload) => {
...
async function loadGiftCatalog() {
    try {
        ...
        const infoEl = document.getElementById('gift-catalog-info');
        if (infoEl) infoEl.innerHTML = '<span class="text-red-400">Error loading catalog</span>';
