...
(() => {
    ...
    function initializeCategoryToggles() {
        const categories = document.querySelectorAll('.sidebar-category[data-category]');
        ...
        categories.forEach(category => {
            ...
            header.setAttribute('aria-expanded', !category.classList.contains('collapsed'));
        ...
        // Re-initialize Lucide icons for chevron icons
        if (typeof lucide !== 'undefined') {
            lucide.createIcons();
        }
    }
    ...
    function toggleCategory(category, categoryId) {
        ...
        if (category.classList.contains('collapsed')) {
            localStorage.setItem(`sidebar-category-${categoryId}`, 'collapsed');
        } else {
            localStorage.removeItem(`sidebar-category-${categoryId}`);
        }

        // Re-initialize Lucide icons for smooth icon transition
        if (typeof lucide !== 'undefined') {
            lucide.createIcons();
        }
    }
