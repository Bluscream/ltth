...
#### 👤 Avatar Auto-Discovery (NEW) 🎉
Automatically detect the current VRChat avatar and its available parameters/actions.

**Features:**
- One-click avatar detection
- Automatic parameter discovery
- Visual indicators for available vs unavailable actions
- Auto-populate avatar list
- Detects: Standard actions, Emotes, Custom parameters, PhysBones

**How to use:**
1. Enable OSCQuery in settings
2. Start OSC-Bridge
3. Load avatar in VRChat
4. Click "🔍 Avatar erkennen" button
5. See available actions instantly!

**API Endpoints:**
- `GET /api/osc/avatar/current` - Get current avatar ID
- `GET /api/osc/avatar/available-actions` - List available actions
- `POST /api/osc/avatar/auto-detect` - Auto-detect and save avatar

**Documentation:** See [AVATAR_AUTO_DISCOVERY.md](./AVATAR_AUTO_DISCOVERY.md) for full details.
