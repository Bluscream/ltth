...
class OpenShockPlugin {
    ...
    async handleTikTokEvent(eventType, eventData) {
        try {
            ...
            // Enhanced logging for follow events
            if (eventType === 'follow') {
                const userId = eventData.userId || eventData.uniqueId || 'unknown';
                const userName = eventData.username || eventData.nickname || 'Unknown';
                this.api.log(`[Follow Event] Received follow from user ${userName} (${userId})`, 'info');
                this.api.log(`[Follow Event] Event data: ${JSON.stringify(eventData)}`, 'debug');
            }
            ...
            if (matches.length === 0) {
                if (eventType === 'gift') {
                    ...
                    // Log when follow events don't match any mappings (could be due to cooldown or conditions)
                    const userId = eventData.userId || eventData.uniqueId || 'unknown';
                    const userName = eventData.username || eventData.nickname || 'Unknown';
                    this.api.log(`[Follow Event] No mappings matched for user ${userName} (${userId}) - likely due to cooldown or conditions`, 'debug');
                }
                return;
            }
