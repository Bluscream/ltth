...
async function loadConfig() {
    try {
        ...
        if (currentConfig.customFishVoices) {
            ...
            console.log('[TTS Config] Loaded config has NO customFishVoices field');
        }
        
        populateConfig(currentConfig);
        
        // Show/hide emotion selector based on current engine
        const selectedEngine = currentConfig.defaultEngine || 'tiktok';
...
function populateConfig(config) {
    ...
    setValue('enableTikTokFallback', config.enableTikTokFallback !== false);
    ...
    setValue('enableFishAudioFallback', config.enableFishAudioFallback || false);
    setValue('enableSiliconFlowFallback', config.enableSiliconFlowFallback || false);
    ...
    // API keys are now managed in the global Settings panel
    ...
    try {
        ...
        const config = {
            ...
            messagePrefixFilter: document.getElementById('messagePrefixFilter').value
                ...
                .filter(p => p.length > 0),
            // Fallback engine activation settings
            enableTikTokFallback: document.getElementById('enableTikTokFallback')?.checked !== false,
            ...
            enableFishAudioFallback: document.getElementById('enableFishAudioFallback')?.checked || false,
            enableSiliconFlowFallback: document.getElementById('enableSiliconFlowFallback')?.checked || false,
            // Language detection settings
            ...
            customFishVoices: currentConfig.customFishVoices || {}
        };

        // API keys are now managed in global Settings panel
        ...
        // Debug: Log customFishVoices after save
        if (currentConfig.customFishVoices) {
            const voiceCount = Object.keys(currentConfig.customFishVoices).length;
        ...
        try {
            await loadVoices();
            console.log('[TTS Config] Voices reloaded after config save');
        } catch (error) {
            console.warn('[TTS Config] Failed to reload voices after save:', error);
...
// ============================================================================
// FISH.AUDIO CUSTOM VOICES MANAGEMENT
// ============================================================================

function renderFishCustomVoices() {
    const container = document.getElementById('fishCustomVoicesList');
    if (!container) return;

    const customVoices = currentConfig.customFishVoices || {};
    ...
    container.innerHTML = voiceIds.map(voiceId => {
        const voice = customVoices[voiceId];
        return `
            <div class="bg-gray-700 rounded-lg p-3 flex items-center justify-between">
                <div class="flex-1">
                    <div class="flex items-center gap-2">
                        <span class="font-semibold text-blue-400">${escapeHtml(voiceId)}</span>
                        <span class="text-xs bg-gray-600 px-2 py-0.5 rounded">${escapeHtml(voice.lang || 'unknown')}</span>
                        ${voice.gender ? `<span class="text-xs bg-gray-600 px-2 py-0.5 rounded">${escapeHtml(voice.gender)}</span>` : ''}
                    </div>
                    <div class="text-xs text-gray-400 mt-1 font-mono break-all">
                        ${escapeHtml(voice.reference_id)}
                    </div>
                    ${voice.name ? `<div class="text-xs text-gray-300 mt-1">${escapeHtml(voice.name)}</div>` : ''}
                </div>
                <button onclick="removeFishCustomVoice('${escapeHtml(voiceId)}')" 
                        class="ml-4 bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm transition flex-shrink-0">
                    🗑️ Remove
                </button>
            </div>
        `;
...
function addFishCustomVoice() {
    const nameInput = document.getElementById('fishCustomVoiceName');
    const referenceIdInput = document.getElementById('fishCustomVoiceReferenceId');
    ...
    if (!currentConfig.customFishVoices) {
        currentConfig.customFishVoices = {};
    }

    currentConfig.customFishVoices[name] = {
        name: name,
        reference_id: referenceId,
        lang: lang,
        gender: 'unknown', // Could be extended later
        model: 's1',
...
async function testTTS() {
    ...
    try {
        const selectedEngine = document.getElementById('defaultEngine')?.value || null;
        const selectedVoice = document.getElementById('defaultVoice')?.value || null;
        const payload = {
            text,
            userId: 'admin',
            username: 'Admin',
        ...
        if (selectedEngine) {
            payload.engine = selectedEngine;
        }
        if (selectedVoice) {
            payload.voiceId = selectedVoice;
        }

        const data = await postJSON('/api/tts/speak', payload);
...
function setupEventListeners() {
    ...
    const addFishCustomVoiceBtn = document.getElementById('addFishCustomVoiceBtn');
    if (addFishCustomVoiceBtn) {
        addFishCustomVoiceBtn.addEventListener('click', addFishCustomVoice);
    }

    // Allow Enter key in Fish voice inputs to trigger add
    const fishVoiceInputs = ['fishCustomVoiceName', 'fishCustomVoiceReferenceId'];
    fishVoiceInputs.forEach(inputId => {
    ...
    // API key toggle handlers removed (now managed in global Settings panel)
