...
const SCENE_LAYERS = ['sky', 'stars', 'grid', 'sigil', 'fog'];
...
async function loadConfig() {
    try {
        const response = await fetch('/api/fireworks-dev/config');
    ...
    try {
        ...
        await fetch('/api/fireworks-dev/trigger', {
            ...
            body: JSON.stringify({
                ...
                theme: config.theme,
                encounterMode: config.encounterMode,
                qualityProfile: config.qualityProfile,
                hudLabel: `${config.theme || 'bossfight'} test`
            })
        });
...
async function triggerFinale() {
    try {
        ...
        await fetch('/api/fireworks-dev/finale', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                intensity: intensity,
...
async function testFollowerFireworks() {
    try {
        await fetch('/api/fireworks-dev/test-follower', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
function updateUI() {
    ...
    updateToggle('pro-mode-toggle', config.proMode === true);
    updateToggle('scene-backdrop-toggle', config.sceneBackdropEnabled !== false);
    updateToggle('shockwave-toggle', config.shockwaveEnabled !== false);
    updateToggle('heat-haze-toggle', config.heatHazeEnabled !== false);
    updateToggle('benchmark-audio-toggle', config.benchmarkMuteAudio !== false);
    document.getElementById('dev-theme').value = config.theme || 'inferno-siege';
    document.getElementById('encounter-mode').value = config.encounterMode || 'skirmish';
    document.getElementById('quality-profile').value = config.qualityProfile || 'ultra';
    document.getElementById('bloom-strength').value = config.bloomStrength || 0.75;
    ...
    SCENE_LAYERS.forEach((layer) => {
        updateToggle(`scene-layer-${layer}-toggle`, config.sceneLayerVisibility[layer] !== false);
        const opacityInput = document.getElementById(`scene-layer-${layer}-opacity`);
        const opacityLabel = document.getElementById(`scene-layer-${layer}-opacity-value`);
        const opacityValue = config.sceneLayerOpacity[layer];
        if (opacityInput) {
...
function ensureSceneConfig() {
    ...
    if (typeof config.sceneBackdropOpacity !== 'number') {
        config.sceneBackdropOpacity = 0.92;
    }
}

function toggleModePanels(isProMode) {
    document.querySelectorAll('[data-panel-mode="pro"]').forEach(panel => {
        panel.dataset.hidden = isProMode ? 'false' : 'true';
    });
}
...
function setupEventListeners() {
    ...
    document.getElementById('pro-mode-toggle')?.addEventListener('click', function() {
        this.classList.toggle('active');
        config.proMode = this.classList.contains('active');
        toggleModePanels(config.proMode);
    });

    document.getElementById('scene-backdrop-toggle')?.addEventListener('click', function() {
        this.classList.toggle('active');
        config.sceneBackdropEnabled = this.classList.contains('active');
    });

    document.getElementById('shockwave-toggle')?.addEventListener('click', function() {
        this.classList.toggle('active');
        config.shockwaveEnabled = this.classList.contains('active');
    });

    document.getElementById('heat-haze-toggle')?.addEventListener('click', function() {
        this.classList.toggle('active');
        config.heatHazeEnabled = this.classList.contains('active');
    });

    document.getElementById('benchmark-audio-toggle')?.addEventListener('click', function() {
        this.classList.toggle('active');
        config.benchmarkMuteAudio = this.classList.contains('active');
    });
    ...
    document.getElementById('dev-theme')?.addEventListener('change', function() {
        config.theme = this.value;
    });

    document.getElementById('encounter-mode')?.addEventListener('change', function() {
        config.encounterMode = this.value;
    });

    document.getElementById('quality-profile')?.addEventListener('change', function() {
        config.qualityProfile = this.value;
    });
    ...
    setupRangeSlider('bloom-strength', 'bloom-strength-value', '', (val) => {
        config.bloomStrength = parseFloat(val);
    });

    setupRangeSlider('scene-backdrop-opacity', 'scene-backdrop-opacity-value', '', (val) => {
        config.sceneBackdropOpacity = parseFloat(val);
    });

    SCENE_LAYERS.forEach((layer) => {
        document.getElementById(`scene-layer-${layer}-toggle`)?.addEventListener('click', function() {
            ensureSceneConfig();
            this.classList.toggle('active');
            config.sceneLayerVisibility[layer] = this.classList.contains('active');
        });

        setupRangeSlider(`scene-layer-${layer}-opacity`, `scene-layer-${layer}-opacity-value`, '', (val) => {
            ensureSceneConfig();
    ...
    document.getElementById('resolution-preset')?.addEventListener('change', function() {
        ...
        if (socket) {
            socket.emit('fireworks-dev:config-update', { 
                config: { 
    ...
    document.getElementById('copy-overlay-url').addEventListener('click', () => {
        const url = window.location.origin + '/fireworks-dev/overlay';
    ...
    document.getElementById('open-overlay').addEventListener('click', () => {
        window.open('/fireworks-dev/overlay', '_blank');
    });
...
async function triggerTestRandom() {
    try {
        await fetch('/api/fireworks-dev/random', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                theme: config.theme,
...
async function applyPreset(presetName) {
    ...
    const benchmarkResults = localStorage.getItem('fireworks-dev-benchmark-results');
    if (benchmarkResults) {
        try {
...
async function startBenchmark() {
    ...
    const overlayUrl = `${window.location.origin}/fireworks-dev/overlay?benchmark=true`;
...
async function restoreBenchmarkPreset() {
    try {
        const response = await fetch('/api/fireworks-dev/benchmark/restore', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function runBenchmarkTest(presetName) {
    ...
    const response = await fetch('/api/fireworks-dev/benchmark/set-preset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function measureFPS() {
    ...
    const fpsTimer = setInterval(async () => {
        try {
            const response = await fetch('/api/fireworks-dev/benchmark/fps');
            const data = await response.json();
...
function saveBenchmarkResults() {
    try {
        localStorage.setItem('fireworks-dev-benchmark-results', JSON.stringify(benchmarkResults));
    } catch (e) {
...
function loadBenchmarkResults() {
    try {
        const saved = localStorage.getItem('fireworks-dev-benchmark-results');
        if (saved) {
