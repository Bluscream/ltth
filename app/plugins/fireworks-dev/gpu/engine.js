...
class FireworksEngine {
    ...
    connectSocket() {
        try {
            ...
            this.socket.on('fireworks-dev:trigger', (data) => {
                this.handleTrigger(data);
            });

            this.socket.on('fireworks-dev:finale', (data) => {
                this.handleFinale(data);
            });

            this.socket.on('fireworks-dev:follower-animation', (data) => {
                this.showFollowerAnimation(data);
            });
            
            this.socket.on('fireworks-dev:get-active-count', () => {
                this.socket.emit('fireworks-dev:active-count-response', { 
                    count: this.fireworks.length,
                    particles: this.getTotalParticles()
                });
            });

            this.socket.on('fireworks-dev:config-update', (data) => {
                if (data.config) {
    ...
    render() {
        ...
        if (now - this.fpsUpdateTime >= 1000) {
            ...
            if (this.socket && this.socket.connected) {
                this.socket.emit('fireworks-dev:fps-update', { fps: this.fps, timestamp: now });
            }
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/abschussgeraeusch.mp3', 'launch-basic');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/abschussgeraeusch2.mp3', 'launch-basic2');
    
    // Basic explosion and rocket sounds
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion.mp3', 'explosion-basic');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/rocket.mp3', 'rocket-basic');
    
    // NEW: Additional explosion sounds with varying intensities
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion_small1.mp3', 'explosion-small');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion_medium.mp3', 'explosion-medium');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion2.mp3', 'explosion-alt1');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion3.mp3', 'explosion-alt2');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion_big.mp3', 'explosion-big');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion_huge.mp3', 'explosion-huge');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/explosion%20Pop%2CSharp%2C.mp3', 'explosion-pop'); // URL encoded filename
    
    // NEW: Crackling sounds for atmospheric effects
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/crackling.mp3', 'crackling-long');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/crackling2.mp3', 'crackling-medium');
    
    // Combined sounds - synchronized launch + explosion in one file
    // These play the entire sequence from launch to explosion
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_crackling_bang.mp3', 'combined-crackling-bang');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_normal-bang.mp3', 'combined-whistle-normal');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_tiny-bang.mp3', 'combined-whistle-tiny1');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_tiny-bang2.mp3', 'combined-whistle-tiny2');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_tiny-bang3.mp3', 'combined-whistle-tiny3');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_tiny-bang4.mp3', 'combined-whistle-tiny4');
    
    // Launch-only sounds (no explosion) - for when we want to play explosion separately
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_mit-pfeifen_no-bang.mp3', 'launch-whistle');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_nocrackling_no-bang.mp3', 'launch-smooth');
    await engine.audioManager.preload('/plugins/fireworks-dev/audio/woosh_abheben_nocrackling_no-bang2.mp3', 'launch-smooth2');
