...
class GoalsEventHandlers {
    ...
    /**
     * Reset goals when stream ends
     * This ensures goals start fresh for the next stream session
     */
        ...
        try {
            ...
            for (const goal of goals) {
                ...
                if (goal.on_reach_action === 'double' || goal.on_reach_action === 'increment') {
                    ...
                    // For other goal types (hide, reset, etc.), just reset the current value
                    // This fixes the bug where goals carried over values from previous stream sessions
                    const updatedGoal = this.db.updateGoal(goal.id, {
                        current_value: goal.start_value
                    });
                    ...
                    this.api.log(`Reset goal "${goal.name}" current value to: ${goal.start_value}`, 'info');
                }
            }
        } catch (error) {
